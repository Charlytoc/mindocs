...
async def upload_files(files: List[UploadFile] = File(...)):
    ...
    os.makedirs(f"uploads/{process_id}", exist_ok=True)

    for file in files:
        with open(f"uploads/{process_id}/{file.filename}", "wb") as buffer:
            shutil.copyfileobj(file.file, buffer)

    return JSONResponse(content={"process_id": process_id})


# the following route will deploy a
